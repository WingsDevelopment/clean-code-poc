...
export const usePaginableSortedData = (
  ...
  defaultRowsPerPage: number = IS_DEV_MODE ? 2 : 10
) => {
  const [page, setPage] = React.useState(1);
  const [rowsPerPage, setRowsPerPage] = React.useState(defaultRowsPerPage);
  const [orderBy, setOrderBy] = React.useState(defaultOrderBy);
  const [isAscending, setIsAscending] = React.useState(true);
  const [dataToShow, setDataToShow] = React.useState<any[] | undefined>(undefined);
  ...
  }, [data, page, rowsPerPage, orderBy, isAscending, getSortedData, getPageData]);

  const setSortBy = (sortBy: string, isAscending: boolean) => {
    setOrderBy(sortBy);
